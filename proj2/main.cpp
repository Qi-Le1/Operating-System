...
void* producer(void *arg) {
  while (true) {
    ...
    if (DEBUG)
      cout<<("producer start signaling")<<endl;
    need_item_cv.signal();
    if (DEBUG)
      cout<<("producer finish signaling")<<endl;

    producer_in_critical_section = false;
    buffer_lock.unlock();
    if (DEBUG)
    cout<<("producer finish the loop")<<endl;

    // Randomly give another thread a chance
    if ((rand() % 100) < RANDOM_YIELD_PERCENT) {
      if (DEBUG)
      cout<<("producer yield the thread")<<endl;
      uthread_yield();
    }
  }
...
void* consumer(void *arg) {
  while (true) {
    if (DEBUG)
    cout<<("consumer start the loop")<<endl;
    buffer_lock.lock();
    ...
    if (item_count == 0) {
      if (DEBUG)
      cout<<("consumer start waiting")<<endl;
      need_item_cv.wait(buffer_lock);
      if (DEBUG)
      cout<<("consumer finish waiting")<<endl;
    }
    ...
    if (DEBUG)
    cout<<("consumer finish signaling")<<endl;

    consumer_in_critical_section = false;
    buffer_lock.unlock();
    if (DEBUG)
    cout<<("consumer finish the loop")<<endl;

    // Randomly give another thread a chance
    if ((rand() % 100) < RANDOM_YIELD_PERCENT) {
      if (DEBUG)
      cout<<("consumer yields")<<endl;
      uthread_yield();
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < producer_count; i++) {
    ...
    if (DEBUG) std::cout<<"producer id:"<<producer_threads[i]<<endl;
  ...
  for (int i = 0; i < consumer_count; i++) {
    ...
    if (DEBUG) std::cout<<"consumer id:"<<consumer_threads[i]<<endl;

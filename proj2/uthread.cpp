...
{
	if (running->getLockCount())
		running->setPriority(RED); // priority ceiling
	else
		_uthread_decrease_priority(running); // decrease priority by one only when no lock is being held
    uthread_yield();
}
...
int uthread_init(int quantum_usecs)
{
	...
	TCB* mainTh = new TCB(0, RED, NULL, NULL, READY);
